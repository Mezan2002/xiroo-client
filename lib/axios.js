...
axiosInstance.interceptors.response.use(
  ...
  (error) => {
    ...
    // Handle 503 Service Unavailable (DB connection issue — do NOT logout)
    if (error.response?.status === 503) {
      // Silently let React Query handle retries
      return Promise.reject(error);
    }
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      // Always clear invalid token from store and storage
      store.dispatch(logout());
      
      const pathname = typeof window !== "undefined" ? window.location.pathname : "";
      
      // Define Protected Regions (Only redirect in these zones)
    ...
    const message = error.response?.data?.message || error.message || "--- System Registry Access Failure ---";
