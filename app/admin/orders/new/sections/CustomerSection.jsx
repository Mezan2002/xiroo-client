...
import { User, Mail, Phone, MapPin, ChevronDown, Search, Truck, Check } from "lucide-react";
...
const Label = ({ children, optional }) => (
  <label className="text-[10px] font-bold text-zinc-900 uppercase block mb-3 tracking-[0.2em] flex items-center gap-2">
    {children}
    {optional && <span className="text-zinc-300 font-normal normal-case tracking-normal text-[9px]">(optional)</span>}
  </label>
);
...
const SubHeader = ({ children }) => (
  <div className="border-b border-zinc-100 pb-4 mb-8">
    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.25em]">{children}</p>
  </div>
);

const LocationDropdown = ({ value, onChange, options, placeholder }) => {
  ...
  const toggle = () => {
    if (!isOpen && ref.current) {
      ...
      setCoords({ top: rect.top + window.scrollY, bottom: rect.bottom + window.scrollY, left: rect.left + window.scrollX, width: rect.width });
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <span className={`text-xl font-light ${value ? "text-black" : "text-zinc-200"}`}>{value || placeholder}</span>
      ...
      {isOpen && typeof document !== "undefined" && createPortal(
        <div
          ...
          style={{ left: coords.left, width: coords.width, ...(openAbove ? { bottom: window.innerHeight - (coords.top - window.scrollY) + 4 } : { top: (coords.bottom - window.scrollY) + 4 }) }}
        >
          <div className="p-3 border-b border-zinc-50 flex items-center gap-3">
            <Search className="w-4 h-4 text-zinc-300" />
            <input autoFocus className="flex-1 outline-none text-sm font-medium" placeholder="search..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <div className="max-h-64 overflow-y-auto">
            {filtered.length > 0 ? filtered.map(o => (
              <div key={o} onClick={() => { onChange(o); setIsOpen(false); setSearch(""); }}
              ...
              <div className="px-5 py-8 text-[11px] font-bold uppercase tracking-widest text-center text-zinc-300">No results</div>
            )}
          </div>
        </div>,
        document.body
...
export default function CustomerSection({ order, setOrder }) {
  const thanas = order.shipping?.district ? (BANGLADESH_LOCATIONS[order.shipping.district] || []) : [];

  const handleShipping = (field, value) => {
    setOrder(prev => ({ ...prev, shipping: { ...prev.shipping, [field]: value } }));
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          <div className="space-y-3">
            ...
            <div className="relative group">
              <input type="text" placeholder="John"
                ...
                onChange={e => setOrder({ ...order, firstName: e.target.value, customer: `${e.target.value} ${order.lastName || ""}`.trim() })}
              />
              <User className="absolute right-0 top-1/2 -translate-y-1/2 text-zinc-300 group-focus-within:text-black" size={18} />
            </div>
          </div>
          <div className="space-y-3">
            ...
            <div className="relative group">
              <input type="text" placeholder="Doe"
                ...
                className="w-full bg-transparent border-b border-zinc-200 focus:border-black outline-none transition-all text-xl font-light py-4 placeholder:text-zinc-200"
                onChange={e => setOrder({ ...order, lastName: e.target.value, customer: `${order.firstName || ""} ${e.target.value}`.trim() })}
              />
              <User className="absolute right-0 top-1/2 -translate-y-1/2 text-zinc-300 group-focus-within:text-black" size={18} />
            </div>
          </div>
          <div className="space-y-3">
            ...
            <div className="relative group">
              ...
              />
              <Phone className="absolute right-0 top-1/2 -translate-y-1/2 text-zinc-300 group-focus-within:text-black" size={18} />
            </div>
          </div>
          <div className="space-y-3">
            ...
            <div className="relative group">
              ...
              />
              <Mail className="absolute right-0 top-1/2 -translate-y-1/2 text-zinc-300 group-focus-within:text-black" size={18} />
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div className="space-y-3">
            ...
            <LocationDropdown
              ...
              options={thanas}
              placeholder={order.shipping?.district ? "Select Thana" : "Select district first"}
            />
          </div>
          <div className="space-y-3 md:col-span-2">
            <Label>Detailed Address</Label>
            <div className="relative group">
              <input type="text" placeholder="Road no, House no, Block, Area..."
                value={order.shipping?.address || ""}
                className="w-full bg-transparent border-b border-zinc-200 focus:border-black outline-none transition-all text-xl font-light py-4 placeholder:text-zinc-200"
                onChange={e => handleShipping("address", e.target.value)}
              />
              <MapPin className="absolute right-0 top-1/2 -translate-y-1/2 text-zinc-300 group-focus-within:text-black" size={18} />
            </div>
          </div>
          <div className="space-y-3">
          ...
          <div className="flex gap-4">
            {deliveryMethods.map((method) => (
              ...
              >
                ...
                <span className={`text-[10px] font-medium mt-2 uppercase tracking-wider ${order.deliveryMethod === method.id ? 'text-zinc-300' : 'text-zinc-400'}`}>
                  {method.desc}
                </span>
              </button>
            ))}
